...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        if self.use_spade:
            ...
            self.mlp_shared = nn.Sequential(
                nn.Conv3d(segmap_ch, nhidden, kernel_size=3, padding=1),
                nn.ReLU()
            )
            self.mlp_gamma = nn.Conv3d(nhidden, self.out_channel, kernel_size=3, padding=1)
            self.mlp_beta = nn.Conv3d(nhidden, self.out_channel, kernel_size=3, padding=1)
    ...
    def forward(self, x, emb, segmap=None):
        ...
        :param segmap: an [N x segmap_ch x D x H x W] Tensor of segmentation map (optional).
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.spade_seg_channel = spade_seg_channel
        ...
        self.input_blocks, ch, input_block_chans = create_encoder(ds, in_channel)
            ...
            for i in range(res_blocks + 1):
                ...
                layers = [
                    ResBlock(
                        ...
                        use_scale_shift_norm=use_scale_shift_norm,
                        use_spade=True,
                        segmap_ch=self.spade_seg_channel,
                    )
                ]
    ...
    def forward(self, x, gammas, seg=None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x D x H x W] Tensor of inputs
        :param gammas: a 1-D batch of gammas.
        :param seg: an [N x segmap_ch x D x H x W] Tensor of segmentation maps (optional).
        :return: an [N x C x D x H x W] Tensor of outputs.
        """
        hs = []
...
if __name__ == '__main__':
    ...
    seg = torch.ones((b, 1, d, h, w)).to(torch.float)
    emb = torch.ones((b, ))
    out = model(x, emb, seg)
